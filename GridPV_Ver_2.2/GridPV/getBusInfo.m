...
for ii=1:length(busNames)
    ...
    Buses(ii).voltagebaseAngle =  angle(voltages(1,busNodes>=1 & busNodes<=3)+1i*voltages(2,busNodes>=1 & busNodes<=3));
    angles(nodes) = Buses(ii).voltagebaseAngle;
    Buses(ii).voltagebaseAngle = angles;
